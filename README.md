...
mkdir /opt/odoo16-custom && cd /opt/odoo16-custom
git clone https://github.com/<OWNER>/<REPO>.git
# resulting folder: /opt/odoo16-custom/pfe-main

# 3. Virtual environment + Python deps
python3.8 -m venv venv
source venv/bin/activate
pip install wheel
pip install -r pfe-main/requirements.txt   # generated by Odoo
...
~/odoo16/odoo-bin -c odoo.conf \
  --addons-path="~/odoo16/addons,/opt/odoo16-custom" \
  -d pfe_db -i pfe_main
```
...
```
Proxmox VE  ➜  VM (Ubuntu 22.04 LTS)
            ➜  PostgreSQL 14
            ➜  Odoo 16 Community + pfe_main add-on
...
This project is licensed under the **GNU LGPL v3.0** – see the [LICENSE](LICENSE) file for details.
